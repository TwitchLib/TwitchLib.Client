...
namespace TwitchLib.Client.Tests
{
    ...
    public class TwitchClient_MessageReceivingTests : ATwitchClientTests<ITwitchClient_MessageReceiving>
    {
        ...
        public void TwitchClient_Raises_Nothing_OnMessageReceived(string message, IrcCommand ircCommand)
        {
            ...
            mock.Setup(c => c.Send(It.Is<string>(msg => String.Equals(msg, message))))
                .Returns(true)
        ...
        public void TwitchClient_Raises_OnUserJoined()
        {
            ...
            mock.InSequence(sequence)
                ...
                .Returns(true)
                .Raises(c => c.OnMessage += null, new OnMessageEventArgs($":tmi.twitch.tv 004 {TWITCH_Username} :-"));
            ...
            mock.InSequence(sequence)
                .Setup(c => c.Send(It.IsAny<string>()))
                .Returns(true)
                .Raises(c => c.OnMessage += null, new OnMessageEventArgs($":{TWITCH_UsernameAnother}!{TWITCH_UsernameAnother}@{TWITCH_UsernameAnother}.tmi.twitch.tv JOIN #{TWITCH_CHANNEL}"));
        ...
        public void TwitchClient_Raises_OnUserLeft()
        {
            ...
            ILogger<ITwitchClient> logger = TestLogHelper.GetLogger<ITwitchClient>();
            ConnectionCredentials connectionCredentials = new ConnectionCredentials(TWITCH_Username, TWITCH_OAuth);
            ITwitchClient client = new TwitchClient(connectionCredentials, communicationClient, logger: logger); IClientMocker.SetIsConnected(Mock.Get(communicationClient), true);
            IClientMocker.SetIsConnected(Mock.Get(communicationClient), true);
            ManualResetEvent pauseCheck = new ManualResetEvent(false);
